...
public class WavySpriteEditor:Editor{
	...
	public override void OnInspectorGUI(){
		...
		//Original size
		if(GUILayout.Button("Scale to original size")){
			float k=Mathf.Max(script.texture.width,script.texture.height)*0.01f;
			script.transform.localScale=new Vector3(k,k,1f);
		}
		//Sprite sorting
		GUILayout.Space(10);
		//Get sorting layers
